...
static volatile unsigned int *spibase = (volatile unsigned int*)0xB0000000;
...
static volatile int TM_FATFS_SD_Stat = STA_NOINIT;	/* Physical drive status */

static int TM_FATFS_SD_CardType;			/* Card type flags */
                     ...
                     )
{
    ...
    return spi_transfer8(spibase,dat);
}
...
)
{
    while (btr) {
        // Optimize this.
        *buff++=spi_transfer8(spibase, 0xFF);
        btr--;
    }
}
...
{
    BYTE d;
    unsigned int timeout = millis() + wt;

    do {
        d = xchg_spi(0xFF);
    } while (d != 0xFF && (millis()<timeout));	/* Wait for card goes ready or timeout */
    if (d == 0xFF) {
...
)
{
	...
	//Timer1 = 200;
        ...
        unsigned timeout = millis() + 200;

	do {							// Wait for DataStart token in timeout of 200ms 
		token = xchg_spi(0xFF);
		// This loop will take a time. Insert rot_rdq() here for multitask envilonment. 
	} while ((token == 0xFF) && (millis()<timeout));
	if (token != 0xFE) {
...
BYTE TM_FATFS_WriteEnabled(void) {
...
DSTATUS SD_disk_initialize (void) {
	...
	if (send_cmd(CMD0, 0) == 1) {				/* Put the card SPI/Idle state */
		...
		unsigned timeout = millis() + 1000;
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			for (n = 0; n < 4; n++) ocr[n] = xchg_spi(0xFF);	/* Get 32 bit return value of R7 resp */
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* Is the card supports vcc of 2.7-3.6V? */
				while ((millis()<timeout) && send_cmd(ACMD41, 1UL << 30)) ;	/* Wait for end of initialization with ACMD41(HCS) */
				if ((millis()<timeout) && send_cmd(CMD58, 0) == 0) {		/* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++) {
						ocr[n] = xchg_spi(0xFF);
					}
			...
			while ((millis()<timeout) && send_cmd(cmd, 0));			/* Wait for end of initialization */
			if ((millis()<timeout) || send_cmd(CMD16, 512) != 0) {	/* Set block length: 512 */
				ty = 0;
			}
		}
	}
